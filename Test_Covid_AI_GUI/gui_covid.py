...
root = Tk()
root.title("Safely Sustain : COVID Detection")
...
Title_Label = Label(Tops,font=('Script MT Bold',20,'bold'),text = "       'Coronavirus Detection' \n\t   using Artificial Intelligence\t\t",pady=9,bg= 'white',fg='red',justify ="center")
...
Label_1 =Label(MainFrame, font=('Comic Sans MS', 17,'bold'), text="\tCOVID-19 Prediction using Chest X-Ray Images",padx=2,pady=2, bg="white",fg ="black")
Label_1.grid(row=0, column=0)

Label_2 =Label(MainFrame, font=('arial', 15,'bold'), text="",padx=2,pady=2, bg="white",fg = "black")
Label_2.grid(row=1, column=0,sticky=W)

Label_9 =Button(MainFrame, font=('arial', 19,'bold'), text="  Select X-ray Image ",padx=2,pady=2, bg="blue",fg = "white",command=fileselector)
Label_9.grid(row=2, column=0)

Label_9 =Button(MainFrame, font=('arial', 19,'bold'), text="  Detect Coronavirus ",padx=2,pady=2, bg="blue",fg = "white",command=predict)
...
img = cv2.imread("./Covid-19image.jpg")
img = cv2.resize(img,(450,200))
cv2.imwrite('Covid-19image.jpg',img)
img = ImageTk.PhotoImage(Image.open("Covid-19image.jpg"))
panel = Label(MainFrame, image = img).grid(row=4,column=0,sticky=E)

img1 = cv2.imread("./SS logo.jpg")
img1 = cv2.resize(img1,(224,170))
cv2.imwrite('SS logo.jpg',img1)
img1 = ImageTk.PhotoImage(Image.open("SS logo.jpg"))
